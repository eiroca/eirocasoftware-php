		...
		switch($state){
			case DOKU_LEXER_ENTER:
				...
				if($match['type']=='internalmedia') {
					global $ID;
					$match['src'] = cleanID($match['src']);
					if($dispMagnify)
						$match['detail'] = ml($match['src'],array('id'=>$ID,'cache'=>'cache'),false);
					
					$page = $match['src'];
					$exists = false;
					resolve_mediaid(getNS($ID), $page, $exists);
					if($exists)		$gimgs = @getImageSize(mediaFN($page));
				}
				else {
					if($dispMagnify)
						$match['detail'] = ml($match['src'],array('cache'=>'cache'),false);
					
					$gimgs = @getImageSize($match['src']);
				}
				
				$match['exist'] = !empty($gimgs);
